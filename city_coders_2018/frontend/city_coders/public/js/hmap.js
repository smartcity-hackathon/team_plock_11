...
L.WebGLHeatMap = L.Renderer.extend({
    ...
    _initContainer: function() {
        ...
        try {
          ...
          console.log(options.gradientTexture)
          console.log(options.alphaRange)
    ...
    draw: function() {
        ...
        if (dataLen) {
            ...
            for (var i = 0; i < dataLen; i++) {
                var dataVal = data[i],
                    latlng = L.latLng(dataVal[0], dataVal[1]),
                    point = map.latLngToContainerPoint(latlng);

                heatmap.addPoint(
                    floor(point.x),
                    floor(point.y),
                    scaleFn(latlng, dataVal[2]),
                    dataVal[3]
                );
            }
    ...
    _scalem: function(latlng, radius) {
